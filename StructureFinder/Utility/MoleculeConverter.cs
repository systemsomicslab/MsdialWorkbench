...
using org.openscience.cdk.normalize;
using System.IO;
using System.Text;
...
namespace Riken.Metabolomics.StructureFinder.Utility
{
    ...
    public sealed class MoleculeConverter
    {
        ...
        public static IAtomContainer DictionaryToAtomContainer(Dictionary<int, AtomProperty> atomDictionary, Dictionary<int, BondProperty> bondDictionary)
        {
            ...
            var iContainer = new org.openscience.cdk.AtomContainer();
        ...
        public static string AtomContainerToSmiles(IAtomContainer container)
        {
            ...
            try {
                implicitHydrizedContainer = ExtAtomContainerManipulator.convertExplicitToImplicitHydrogens(container);
                smiles = new SmilesGenerator().createSMILES(implicitHydrizedContainer);
            }
        ...
        private static void readPubChemMetadata(IAtomContainer originalCompound, out string title, out string id, out string inchikey) 
        {
            ...
            if (originalCompound.getProperty("PUBCHEM_IUPAC_TRADITIONAL_NAME") != null)
                titleCandidate = originalCompound.getProperty("PUBCHEM_IUPAC_TRADITIONAL_NAME").ToString();
            else if (originalCompound.getProperty("PUBCHEM_IUPAC_OPENEYE_NAME") != null)
                titleCandidate = originalCompound.getProperty("PUBCHEM_IUPAC_OPENEYE_NAME").ToString();
            else if (originalCompound.getProperty(CDKConstants.TITLE) != null)
                titleCandidate = originalCompound.getProperty(CDKConstants.TITLE).ToString();
            
            if (originalCompound.getProperty("PUBCHEM_COMPOUND_CID") != null)
                id = originalCompound.getProperty("PUBCHEM_COMPOUND_CID").ToString();
            if (originalCompound.getProperty("PUBCHEM_IUPAC_INCHIKEY") != null) {
                inchikey = originalCompound.getProperty("PUBCHEM_IUPAC_INCHIKEY").ToString();
                var shortInchiKey = inchikey.Split('-')[0];
