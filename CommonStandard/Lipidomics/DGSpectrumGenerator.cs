        ...
        private SpectrumPeak[] GetDGSpectrum(ILipid lipid, AdductIon adduct)
        {
            ...
            if (adduct.AdductIonName == "[M+NH4]+")
            {
                ...
                (
                     new[]
                     {
                        new SpectrumPeak((float)(adduct.ConvertToMz(lipid.Mass) - H2O), 150f, "Precursor-H2O"),
                        new SpectrumPeak((float)(lipid.Mass + MassDiffDictionary.ProtonMass), 150f, "[M+H]+"),
                        new SpectrumPeak((float)(lipid.Mass + MassDiffDictionary.ProtonMass-H2O), 150f, "[M+H]+ -H2O"),
                     }
                );
            }
            else if (adduct.AdductIonName == "[M+H]+")
            {
                ...
                (
                     new[]
                     {
                        new SpectrumPeak((float)(adduct.ConvertToMz(lipid.Mass) - H2O), 100f, "Precursor-H2O"),
                     }
                );
            }
            ...
            if (adduct.AdductIonName == "[M+Na]+")
            {
                ...
                (
                     new[]
                     {
                        ...
                        new SpectrumPeak((float)(lipidMass - chainMass - MassDiffDictionary.HydrogenMass - MassDiffDictionary.OxygenMass), 200f, $"-{acylChain}-O") { SpectrumComment = SpectrumComment.acylchain },
                     }
                );
            }
            else
