...
namespace CompMs.Common.DataObj.Property {
    ...
    public sealed class FormulaCalculateUtility {
        ...
        public static double GetExactMass(int cnum, int hnum, int nnum, int onum, int pnum, int snum, int fnum, int clnum, int brnum, int inum, int sinum,
            ...
            var hMass = AtomMass.cMass; if (hLabelMass > 0) hMass = hLabelMass;
            var nMass = AtomMass.cMass; if (nLabelMass > 0) nMass = nLabelMass;
            var oMass = AtomMass.cMass; if (oLabelMass > 0) oMass = oLabelMass;
            var pMass = AtomMass.cMass; if (pLabelMass > 0) pMass = pLabelMass;
            var sMass = AtomMass.cMass; if (sLabelMass > 0) sMass = sLabelMass;
            var fMass = AtomMass.cMass; if (fLabelMass > 0) fMass = fLabelMass;
            var clMass = AtomMass.cMass; if (clLabelMass > 0) clMass = clLabelMass;
            var brMass = AtomMass.cMass; if (brLabelMass > 0) brMass = brLabelMass;
            var iMass = AtomMass.cMass; if (iLabelMass > 0) iMass = iLabelMass;
            var siMass = AtomMass.cMass; if (siLabelMass > 0) siMass = siLabelMass;
