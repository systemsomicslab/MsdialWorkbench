...
namespace Rfx.Riken.OsakaUniv
{
    public sealed class FragmentAssigner
    {
        ...
        public static List<ProductIon> FastFragmnetAssigner(List<Peak> peaklist, List<ProductIon> productIonDB, 
            Formula formula, double ms2Tol, MassToleranceType massTolType, AdductIon adductIon)
        {
            ...
            foreach (var peak in peaklist)
            {
                ...
                if (Math.Abs(mass - formula.Mass - adductIon.AdductIonAccurateMass) < massTol)
                {
                    ...
                    productIons.Add(new ProductIon() { Formula = nFormula, Mass = peak.Mz, MassDiff = formula.Mass + adductIon.AdductIonAccurateMass - mass, Intensity = peak.Intensity });
                    continue;
                }

                //library search
                var fragmentFormulas = getFormulaCandidatesbyLibrarySearch(formula, adductIon.IonMode, peak.Mz, massTol, productIonDB);
        ...
        /// <param name="peaklist"></param>
        /// <returns></returns>
        public static List<NeutralLoss> GetNeutralLossList(List<Peak> peaklist, double precurosrMz, double masstol)
        {
            if (peaklist == null || peaklist.Count == 0) return new List<NeutralLoss>();

            var neutralLosslist = new List<NeutralLoss>();
            var monoIsotopicPeaklist = new List<Peak>();
            var maxNeutralLoss = 1000;

            foreach (var peak in peaklist) if (peak.Comment == "M") monoIsotopicPeaklist.Add(peak);
            monoIsotopicPeaklist = monoIsotopicPeaklist.OrderByDescending(n => n.Mz).ToList();

            var highestMz = monoIsotopicPeaklist[0].Mz;
            if (Math.Abs(highestMz - precurosrMz) > masstol)
                monoIsotopicPeaklist.Insert(0, new Peak() { Mz = precurosrMz, Intensity = 1, Comment = "Insearted precursor" });
            ...
            for (int i = 0; i < monoIsotopicPeaklist.Count; i++)
            {
                for (int j = i + 1; j < monoIsotopicPeaklist.Count; j++)
                {
                    ...
                    if (monoIsotopicPeaklist[i].Mz - monoIsotopicPeaklist[j].Mz < 12 - masstol) continue;
        ...
        /// <returns></returns>
        public static List<Peak> GetRefinedPeaklist(List<Peak> peaklist, double relativeAbundanceCutOff, double precursorMz,
            double ms2Tol, MassToleranceType massTolType, bool isRemoveIsotopes = false, bool removeAfterPrecursor = true)
        {
            if (peaklist == null || peaklist.Count == 0) return new List<Peak>();
            double maxIntensity = getMaxIntensity(peaklist);
            var refinedPeaklist = new List<Peak>();
            var peakListMax = 1000;

            foreach (var peak in peaklist)
            {
                if (removeAfterPrecursor && peak.Mz > precursorMz + 0.01) break;
                if (peak.Intensity >= maxIntensity * relativeAbundanceCutOff * 0.01) { refinedPeaklist.Add(new Peak() { Mz = peak.Mz, Intensity = peak.Intensity, Comment = string.Empty }); }
            }
            ...
            if (isRemoveIsotopes == false) {
                if (refinedPeaklist.Count > peakListMax) {
                    ...
                    var filteredList = new List<Peak>();
                    foreach (var peak in refinedPeaklist) {
                        filteredList.Add(peak);
                        if (filteredList.Count > peakListMax) return filteredList.OrderBy(n => n.Mz).ToList();
                    }
                }
                else {
                ...
                var isotopeRemovedRefinedPeaklist = new List<Peak>();
                ...
                if (isotopeRemovedRefinedPeaklist.Count > peakListMax) {
                    ...
                    foreach (var peak in isotopeRemovedRefinedPeaklist) {
                        ...
                        if (filteredList.Count > peakListMax) {
                            return filteredList.OrderBy(n => n.Mz).ToList();
                        }
                    }
                }
                else {
        ...
        public static List<Peak> GetRefinedPeaklist(List<Peak> peaklist, double precursorMz) {
            if (peaklist == null || peaklist.Count == 0) return new List<Peak>();
            var refinedPeaklist = new List<Peak>();

            foreach (var peak in peaklist) {
                if (peak.Mz > precursorMz + 0.01) break;
                refinedPeaklist.Add(new Peak() { Mz = peak.Mz, Intensity = peak.Intensity, Comment = peak.Comment });
            }
            return refinedPeaklist;
        }
        ...
        /// <param name="precursorMz"></param>
        /// <returns></returns>
        public static List<Peak> GetRefinedPeaklist(List<Peak> peaklist, double relativeAbundanceCutOff, double absoluteAbundanceCutOff,
            double precursorMz, double ms2Tol, MassToleranceType massTolType)
        {
            ...
            foreach (var peak in peaklist) {
                if (peak.Mz > precursorMz + 0.01) break;
                if (peak.Intensity >= maxIntensity * relativeAbundanceCutOff * 0.01 && peak.Intensity > absoluteAbundanceCutOff) { refinedPeaklist.Add(new Peak() { Mz = peak.Mz, Intensity = peak.Intensity, Comment = string.Empty }); }
            }
        ...
        /// <returns></returns>
        public static List<Peak> GetCentroidMsMsSpectrum(RawData rawData)
        {
            List<Peak> ms2Peaklist;

            if (rawData.SpectrumType == DataType.Centroid) ms2Peaklist = rawData.Ms2Spectrum.PeakList;
            else ms2Peaklist = SpectralCentroiding.Centroid(rawData.Ms2Spectrum.PeakList);

            if (ms2Peaklist == null) return new List<Peak>();
        ...
        /// <param name="rawData"></param>
        /// <returns></returns>
        public static List<Peak> GetCentroidMs1Spectrum(RawData rawData)
        {
            List<Peak> ms1Peaklist;

            if (rawData.SpectrumType == DataType.Centroid) ms1Peaklist = rawData.Ms1Spectrum.PeakList;
            else ms1Peaklist = SpectralCentroiding.Centroid(rawData.Ms1Spectrum.PeakList);

            if (ms1Peaklist == null) return new List<Peak>();
        ...
        /// <returns></returns>
        public static List<Peak> GetCentroidSpectrum(List<Peak> peaklist, DataType dataType, double threshold)
        {
            List<Peak> cPeaklist;
            ...
            if (cPeaklist == null) return new List<Peak>();
        ...
        private static List<Formula> getValenceCheckedFragmentFormulaList(Formula formula, IonMode ionMode, double mass, double massTol)
        {
            ...
            double maxHmass = hMass * (formula.Hnum + hydrogen);
            double maxCHmass = maxHmass + cMass * formula.Cnum;
            double maxNCHmass = maxCHmass + nMass * formula.Nnum;
            double maxONCHmass = maxNCHmass + oMass * formula.Onum;
            double maxFONCHmass = maxONCHmass + fMass * formula.Fnum;
            double maxSiFONCHmass = maxFONCHmass + siMass * formula.Sinum;
            double maxPSiFONCHmass = maxSiFONCHmass + pMass * formula.Pnum;
            double maxSPSiFONCHmass = maxPSiFONCHmass + sMass * formula.Snum;
            double maxClSPSiFONCHmass = maxSPSiFONCHmass + clMass * formula.Clnum;
            double maxBrClSPSiFONCHmass = maxClSPSiFONCHmass + brMass * formula.Brnum;
           
            for (int inum = 0; inum <= formula.Inum; inum++)
            {
                if ((double)inum * iMass + maxBrClSPSiFONCHmass < mass - massTol) continue;
                if ((double)inum * iMass > mass + massTol) break;
                var uImass = (double)inum * iMass;
                
                for (int brnum = 0; brnum <= formula.Brnum; brnum++)
                {
                    if (uImass + (double)brnum * brMass + maxClSPSiFONCHmass < mass - massTol) continue;
                    if (uImass + (double)brnum * brMass > mass + massTol) break;
                    var uBrmass = uImass + (double)brnum * brMass;

                    for (int clnum = 0; clnum <= formula.Clnum; clnum++)
                    {
                        if (uBrmass + (double)clnum * clMass + maxSPSiFONCHmass < mass - massTol) continue;
                        if (uBrmass + (double)clnum * clMass > mass + massTol) break;
                        var uClmass = uBrmass + (double)clnum * clMass;

                        for (int snum = 0; snum <= formula.Snum; snum++)
                        {
                            if (uClmass + (double)snum * sMass + maxPSiFONCHmass < mass - massTol) continue;
                            if (uClmass + (double)snum * sMass > mass + massTol) break;
                            var uSmass = uClmass + (double)snum * sMass;

                            for (int pnum = 0; pnum <= formula.Pnum; pnum++)
                            {
                                if (uSmass + (double)pnum * pMass + maxSiFONCHmass < mass - massTol) continue;
                                if (uSmass + (double)pnum * pMass > mass + massTol) break;
                                var uPmass = uSmass + (double)pnum * pMass;

                                for (int sinum = 0; sinum <= formula.Sinum; sinum++)
                                {
                                    if (uPmass + (double)sinum * siMass + maxFONCHmass < mass - massTol) continue;
                                    if (uPmass + (double)sinum * siMass > mass + massTol) break;
                                    var uSimass = uPmass + (double)sinum * siMass;

                                    for (int fnum = 0; fnum <= formula.Fnum; fnum++)
                                    {
                                        if (uSimass + (double)fnum * fMass + maxONCHmass < mass - massTol) continue;
                                        if (uSimass + (double)fnum * fMass > mass + massTol) break;
                                        var uFmass = uSimass + (double)fnum * fMass;

                                        for (int onum = 0; onum <= formula.Onum; onum++)
                                        {
                                            if (uFmass + (double)onum * oMass + maxNCHmass < mass - massTol) continue;
                                            if (uFmass + (double)onum * oMass > mass + massTol) break;
                                            var uOmass = uFmass + (double)onum * oMass;

                                            for (int nnum = 0; nnum <= formula.Nnum; nnum++)
                                            {
                                                if (uOmass + (double)nnum * nMass + maxCHmass < mass - massTol) continue;
                                                if (uOmass + (double)nnum * nMass > mass + massTol) break;
                                                var uNmass = uOmass + (double)nnum * nMass;

                                                for (int cnum = 0; cnum <= formula.Cnum; cnum++)
                                                {
                                                    if (uNmass + (double)cnum * cMass + maxHmass < mass - massTol) continue;
                                                    if (uNmass + (double)cnum * cMass > mass + massTol) break;
                                                    var uCmass = uNmass + (double)cnum * cMass;

                                                    for (int hnum = 0; hnum <= formula.Hnum + hydrogen; hnum++)
                                                    {
                                                        if (uCmass + (double)hnum * hMass < mass - massTol) continue;
                                                        if (uCmass + (double)hnum * hMass > mass + massTol) break;
        ...
        private static List<Peak> isotopicPeakAssignmnet(List<Peak> peaklist, double massTol, MassToleranceType massTolType)
        {
            var c13_c12_Ratio = 0.010815728;
            
            for (int i = 0; i < peaklist.Count; i++)
            {
                var mass = peaklist[i].Mz;
                ...
                else
                {
                    ...
                    {
                        if (mass + MassDiffDictionary.C13_C12_Plus_C13_C12 + ms2Tol <= peaklist[j].Mz) break;
                        ...
                        if (mass + MassDiffDictionary.C13_C12 - ms2Tol < peaklist[j].Mz && 
                            peaklist[j].Mz < mass + MassDiffDictionary.C13_C12 + ms2Tol &&
                            m1Intensity > peaklist[j].Intensity) {
                            peaklist[j].Comment = "M+1";
                            m2Intensity += peaklist[j].Intensity;
                        }

                        if (mass + MassDiffDictionary.N15_N14 - ms2Tol < peaklist[j].Mz &&
                            peaklist[j].Mz < mass + MassDiffDictionary.N15_N14 + ms2Tol &&
                        ...
                        if (m2Intensity > 0.0 && 
                            mass + MassDiffDictionary.C13_C12_Plus_C13_C12 - ms2Tol < peaklist[j].Mz && 
                            peaklist[j].Mz < mass + MassDiffDictionary.C13_C12_Plus_C13_C12 + ms2Tol &&
                            m2Intensity > peaklist[j].Intensity) {
                            peaklist[j].Comment = "M+2";
                        }

                        if (mass + MassDiffDictionary.S34_S32 - ms2Tol < peaklist[j].Mz && 
                            peaklist[j].Mz < mass + MassDiffDictionary.S34_S32 + ms2Tol && 
                            m1Intensity * 0.3 > peaklist[j].Intensity)
                            peaklist[j].Comment = "M+2";

                        if (mass + MassDiffDictionary.O18_O16 - ms2Tol < peaklist[j].Mz && 
                            peaklist[j].Mz < mass + MassDiffDictionary.O18_O16 + ms2Tol && 
        ...
        public static List<AnnotatedIon> GetAnnotatedIon(List<Peak> peaklist, AdductIon mainAdduct, List<AdductIon> referenceAdductTypeList, double precursorMz, double massTol, MassToleranceType massTolType) {
            var annotations = new List<AnnotatedIon>();
            foreach(var peak in peaklist) {
                annotations.Add(new AnnotatedIon() { AccurateMass = peak.Mz });
            }
        ...
        public static void AnnotateIsotopes(List<Peak> peaklist, List<AnnotatedIon> annotations, double massTol, MassToleranceType massTolType) {
            var c13_c12_Ratio = 0.010815728;
            for (int i = 0; i < peaklist.Count; i++) {
                var mass = peaklist[i].Mz;
                var m1Intensity = peaklist[i].Intensity;
                ...
                if (annotations[i].PeakType == AnnotatedIon.AnnotationType.Product) {
                    ...
                    for (int j = i; j < peaklist.Count; j++) {
                        if (mass + MassDiffDictionary.C13_C12_Plus_C13_C12 + ms2Tol <= peaklist[j].Mz) break;
                        if (m2mass > 0 && m2mass + MassDiffDictionary.C13_C12_Plus_C13_C12 + ms2Tol <= peaklist[j].Mz) m2Intensity = 0;
                        ...
                        else if (mass + MassDiffDictionary.N15_N14 - ms2Tol < peaklist[j].Mz &&
                            peaklist[j].Mz < mass + MassDiffDictionary.N15_N14 + ms2Tol &&
                            m1Intensity > peaklist[j].Intensity) {
                        ...
                        else if (mass + MassDiffDictionary.S34_S32 - ms2Tol < peaklist[j].Mz &&
                            peaklist[j].Mz < mass + MassDiffDictionary.S34_S32 + ms2Tol &&
                            m1Intensity * 0.3 > peaklist[j].Intensity) {
                            annotations[j].SetIsotope(mass, peaklist[j].Intensity, m1Intensity, "S34", 2);
                        }

                        else if (mass + MassDiffDictionary.O18_O16 - ms2Tol < peaklist[j].Mz &&
                            peaklist[j].Mz < mass + MassDiffDictionary.O18_O16 + ms2Tol &&
                            m1Intensity * 0.1 > peaklist[j].Intensity) {
                            annotations[j].SetIsotope(mass, peaklist[j].Intensity, m1Intensity, "S34", 2);
                        }
        ...
        public static void AnnotateAdducts(List<Peak> peaklist, List<AnnotatedIon> annotations,
            ...
            for (var i = 0; i < peaklist.Count; i++) {
                ...
                var centralExactMass = MolecularFormulaUtility.ConvertPrecursorMzToExactMass(peak.Mz,
                ...
                for (var j = 0; j < peaklist.Count; j ++) {
                    ...
                    foreach (var targetAdduct in referenceAdductIons) {
                        if (Math.Abs(peak.Mz - precursorMz) > massTol2) {
                            if (targetAdduct.AdductIon.AdductIonXmer > 1) continue;
                        }

                        var adductTol = MolecularFormulaUtility.ConvertPpmToMassAccuracy(targetPeak.Mz, ppm);
                        
                        if (Math.Abs(targetPeak.Mz - targetAdduct.AccurateMass) < adductTol) {
        ...
        private static double getMaxIntensity(List<Peak> peaklist)
        {
