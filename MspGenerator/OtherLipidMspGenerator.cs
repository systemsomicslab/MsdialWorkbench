...
namespace CompMs.MspGenerator
{
    public class OtherLipidMspGenerator
    {
        ...
        public static void fahfahfaGenerator(List<string> Chain1, List<string> chain2, List<string> chain3, string lipidClass, string output)
        {
            var wholeChainList = new List<string>();
            ...
            var adducts = adductDic.lipidClassAdductDic[lipidClass];
            var baseChainDic = AcylChainDic.HydroxyFaChainDictionary;
            var extraChainDic = AcylChainDic.FattyAcylChainDictionary;
            foreach (var adductIon in adducts)
            {
                var adduct = adductDic.adductIonDic[adductIon];
                ...
                using (var sw = new StreamWriter(output + "\\" + filename, false, Encoding.ASCII))
                {
                    for (int i = 0; i < wholeChainList.Count; i++)
                    {
                        ...
                        var baseChainSmiles = baseChain[3].Replace("%10", "%20") + "O";
                        var hfaChainSmiles = hfaChain[3] + "%20.";

                        var extraChainSmiles = extraChain[3];

                        var name = "";

                        var exportLipidClassName = "FAHFAHFA";
                        var rawSmiles = extraChainSmiles + "%10." + hfaChainSmiles + baseChainSmiles;
                        var meta = Common.getMetaProperty(rawSmiles);

                        // fragment
                        var fragmentList = new List<string>();
                        name = "FAHFAHFA" + " " + chainArray[2] + "/" + hfaChainString + "/" + baseChainString;
                        if (chainNameList.Contains(name))
                        {
                            continue;
                        }
                        OtherLipidFragmentation.fahfahfaFragment(fragmentList, adduct.AdductIonName, meta.ExactMass, baseChainCarbon, baseChainDouble, extraChainCarbon, extraChainDouble, baseChainOxposition);
            ...
            foreach (var adductIon in adducts)
            {
                ...
                using (var sw = new StreamWriter(output + "\\" + filename, false, Encoding.ASCII))
                {
                    for (int i = 0; i < wholeChainList.Count; i++)
                    {
                        ...
                        OtherLipidFragmentation.fahfahfaDmedFragment(fragmentList, adduct.AdductIonName, meta.ExactMass, baseChainCarbon, baseChainDouble, extraChainCarbon, extraChainDouble, baseChainOxposition);
                    ...
                    for (int i = 0; i < wholeChainList.Count; i++)
                    {
                        ...
                        switch (lipidClass)
                        {
                            ...
                            case "LCAE":
                                ...
                                if (chain1Ox > 0)
                                {
                                    name = name + ";O" + (chain1Ox.ToString() == "1" ? "" : chain1Ox.ToString());
                                }
                                break;
