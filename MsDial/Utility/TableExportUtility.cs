...
namespace Rfx.Riken.OsakaUniv
{
    public sealed class TableExportUtility
    {
        ...
        public static void SavePcaTableAsTextFormat(string exportFilePath, PrincipalComponentAnalysisResult pcaBean)
        {
            using (StreamWriter sw = new StreamWriter(exportFilePath, false, Encoding.ASCII))
            {
                //header set
                sw.WriteLine("Contribution");
                for (int i = 0; i < pcaBean.ContributionCollection.Count; i++)
                    sw.WriteLine("PC score: " + (i + 1).ToString() + "\t" + pcaBean.ContributionCollection[i]);

                //header set
                sw.WriteLine("Score");
                sw.Write("" + "\t");
                for (int i = 0; i < pcaBean.ScorePointsCollection.Count; i++)
                {
                    if (i == pcaBean.ScorePointsCollection.Count - 1) { sw.Write("PC score: " + (i + 1).ToString() + "\r\n"); break; }
                    sw.Write("PC score: " + (i + 1).ToString() + "\t");
                }

                for (int i = 0; i < pcaBean.ScorePointsCollection[0].Length; i++)
                {
                    sw.Write(pcaBean.ScoreLabelCollection[i] + "\t");
                    for (int j = 0; j < pcaBean.ScorePointsCollection.Count; j++)
                    {
                        if (j == pcaBean.ScorePointsCollection.Count - 1)
                        {
                            sw.Write(pcaBean.ScorePointsCollection[j][i] + "\r\n");
                            break;
                        }
                        else
                            sw.Write(pcaBean.ScorePointsCollection[j][i] + "\t");
                ...
                sw.Write("" + "\t");
                for (int i = 0; i < pcaBean.LoadingPointsCollection.Count; i++)
                {
                    if (i == pcaBean.LoadingPointsCollection.Count - 1) { sw.Write("PC Loading: " + (i + 1).ToString() + "\r\n"); break; }
                    sw.Write("PC Loading: " + (i + 1).ToString() + "\t");
                }

                for (int i = 0; i < pcaBean.LoadingPointsCollection[0].Length; i++)
                {
                    sw.Write(pcaBean.LoadingLabelCollecction[i] + "\t");
                    for (int j = 0; j < pcaBean.LoadingPointsCollection.Count; j++)
                    {
                        if (j == pcaBean.LoadingPointsCollection.Count - 1)
                        {
                            sw.Write(pcaBean.LoadingPointsCollection[j][i] + "\r\n");
                            break;
                        }
                        else
                            sw.Write(pcaBean.LoadingPointsCollection[j][i] + "\t");
        ...
        public static void CopyToClipboardPcaTableAsTextFormat(PrincipalComponentAnalysisResult pcaBean)
        {
            ...
            text += "Contribution" + "\r\n";
            for (int i = 0; i < pcaBean.ContributionCollection.Count; i++)
                text += "PC score: " + (i + 1).ToString() + "\t" + pcaBean.ContributionCollection[i];
            ...
            for (int i = 0; i < pcaBean.ScorePointsCollection.Count; i++)
            {
                if (i == pcaBean.ScorePointsCollection.Count - 1) { text += "PC score: " + (i + 1).ToString() + "\r\n"; break; }
                text += "PC score: " + (i + 1).ToString() + "\t";
            }

            for (int i = 0; i < pcaBean.ScorePointsCollection[0].Length; i++)
            {
                text += pcaBean.ScoreLabelCollection[i] + "\t";
                for (int j = 0; j < pcaBean.ScorePointsCollection.Count; j++)
                {
                    if (j == pcaBean.ScorePointsCollection.Count - 1)
                    {
                        text += pcaBean.ScorePointsCollection[j][i] + "\r\n";
                        break;
                    }
                    else
                        text += pcaBean.ScorePointsCollection[j][i] + "\t";
            ...
            for (int i = 0; i < pcaBean.LoadingPointsCollection.Count; i++)
            {
                if (i == pcaBean.LoadingPointsCollection.Count - 1) { text += "PC Loading: " + (i + 1).ToString() + "\r\n"; break; }
                text += "PC Loading: " + (i + 1).ToString() + "\t";
            }

            for (int i = 0; i < pcaBean.LoadingPointsCollection[0].Length; i++)
            {
                text += pcaBean.LoadingLabelCollecction[i] + "\t";
                for (int j = 0; j < pcaBean.LoadingPointsCollection.Count; j++)
                {
                    if (j == pcaBean.LoadingPointsCollection.Count - 1)
                    {
                        text += pcaBean.LoadingPointsCollection[j][i] + "\r\n";
                        break;
                    }
                    else
                        text += pcaBean.LoadingPointsCollection[j][i] + "\t";
