...
{
    [TestClass()]
    public class MsdialDataStorageTests
    {
        [TestMethod()]
        public async Task SaveTest() {
            ...
            dbs.AddMoleculeDataBase(
                ...
                new List<IAnnotatorParameterPair<MoleculeDataBase>> {
                    new MetabolomicsAnnotatorParameterPair(annotator.Save(), new AnnotationQueryFactory(annotator, storage.ParameterBase.PeakPickBaseParam, searchParameter, ignoreIsotopicPeak: true))
                }
            );
            ...
            using (var manager = ZipStreamManager.OpenGet(memory)) {
                var actual = await MsdialDataStorage.Serializer.LoadAsync(manager, "Test", null, "TestFolder");
