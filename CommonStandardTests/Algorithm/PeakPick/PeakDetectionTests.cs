...
namespace CompMs.Common.Algorithm.PeakPick.Tests
{
    ...
    public class PeakDetectionTests
    {
        ...
        {
            var results = Enumerable.Range(start, radian * 2 + 1).Select(i => ChromatogramPeak.Create(i, i, i - start, mass, baseIntensity, new RetentionTime(baseTime + i * 0.002))).ToArray();
