...
{
    [TestClass()]
    public class MsdialGcmsDataStorageTests
    {
        [TestMethod()]
        public async Task SaveTest() {
            ...
            var db = new MoleculeDataBase(new[] { new MoleculeMsReference { DatabaseID = 7, Name = "TestDBRef" } }, "DummyDB", DataBaseSource.Msp, SourceType.MspDB, "TextDBPath");
            ...
            using (var manager = ZipStreamManager.OpenGet(memory)) {
                var actual = await MsdialGcmsDataStorage.Serializer.LoadAsync(manager, "Test", null, "TestFolder");
