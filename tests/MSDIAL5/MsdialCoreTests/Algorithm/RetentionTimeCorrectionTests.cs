...
public class RetentionTimeCorrectionTests
{
    ...
    public void GetRetentionTimeCorrectionBean_SampleMinusAverage_PartialStandardUsed() {
        ...
        var stds = new List<StandardPair>
            {
                new() { SamplePeakAreaBean = new() { ChromXs = new ChromXs(new RetentionTime(3d)) }, Reference = references[0] },
                new() { SamplePeakAreaBean = new() { ChromXs = new ChromXs(new RetentionTime(2d)) }, Reference = references[1] },
                new() { SamplePeakAreaBean = new() { ChromXs = new ChromXs(new RetentionTime(1d)) }, Reference = references[2] },
            };
        var commons = new List<CommonStdData> { new(references[2]) { AverageRetentionTime = .8f }, new(references[1]) { AverageRetentionTime = 1.5f }, };
