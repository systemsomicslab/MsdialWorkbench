...
{
    /// <summary>
    /// The LC-MS annotators record which terms they computed. This is the mode the public
    /// repository reanalysis runs in.
    /// </summary>
    /// <remarks>
    ...
    ///
    /// The MSP annotator here has no initializer of its own: it delegates to MsReferenceScorer, so
    /// this is also the witness that the shared scorer reaches the LC-MS path.
    /// </remarks>
    [TestClass()]
    public class MeasuredTermsAreRecordedTests
    {
        ...
        public void TheMspAnnotatorRecordsEveryTermItComputes() {
            ...
            var annotator = new LcmsMspAnnotator(Database(SourceType.MspDB), parameter, TargetOmics.Metabolomics, "MspDB", -1);
        ...
        [TestMethod()]
        public void TheMspAnnotatorLeavesTimeClearWhenTheRunDoesNotScoreIt() {
            var parameter = Parameter();
        ...
        [TestMethod()]
        public void TheTextDbAnnotatorRecordsMassIsotopeAndTimeButNotSpectrum() {
            var parameter = Parameter();
            var annotator = new LcmsTextDBAnnotator(Database(SourceType.TextDB), parameter, "TextDB", -1);

            var result = annotator.CalculateScore(Query(annotator, parameter), Reference());

            Assert.AreEqual(
                MeasuredTerms.AccurateMass | MeasuredTerms.RetentionTime | MeasuredTerms.Isotope,
                result.MeasuredTerms,
                "a text database holds no reference spectrum, so nothing here opens one");
        }

        [TestMethod()]
        public void TheMspAnnotatorLeavesTimeClearWhenTheReferenceHasNone() {
            // Distinct from the run not scoring time: here the run does score it, the scorer does
            // enter the block, and the reference has no retention time to compare. The value it
            // stores in RtSimilarity is nevertheless not zero, because this path uses the
            // unguarded Gaussian overload. The flag is what says no comparison happened.
            var parameter = Parameter();
            var annotator = new LcmsMspAnnotator(Database(SourceType.MspDB), parameter, TargetOmics.Metabolomics, "MspDB", -1);
        ...
        public void TheTextDbAnnotatorLeavesIsotopeClearWithNoIsotopicPeaks() {
            ...
            Assert.AreEqual(MeasuredTerms.AccurateMass | MeasuredTerms.RetentionTime, result.MeasuredTerms);
        }
        ...
        public void TheMspAnnotatorRecordsRuleBasedOnALipidomicsRun() {
            var parameter = Parameter();
            var annotator = new LcmsMspAnnotator(Database(SourceType.MspDB), parameter, TargetOmics.Lipidomics, "MspDB", -1);
