...
namespace CompMs.Common.StructureFinder
{
    public sealed class FragmentPeakMatcher
    {
        ...
        /// </summary>
        public static List<PeakFragmentPair> GetSpectralAssignmentResult(Structure structure, List<Fragment> fragments, List<SpectrumPeak> peaks,
            AdductIon adduct, int treeDepth, double massTolerance, MassToleranceType massTolType, IonMode ionMode, List<FragmentLibrary> fragmentDB = null)
        {
            ...
            peaks = peaks.OrderByDescending(n => n.Mass).ToList();
            ...
            for (int i = 0; i < peaks.Count; i++)
            {
                ...
                var massTol = massTolType == MassToleranceType.Da ? massTolerance : MolecularFormulaUtility.ConvertPpmToMassAccuracy(peak.Mass, massTolerance);
                var fragmentStartIndex = getFragmentStartIndex(fragments, peak.Mass - fragmentPlusShift, massTol);
                ...
                for (int j = fragmentStartIndex; j < fragments.Count; j++)
                {
                    ...
                    if (fragment.ExactMass + fragmentPlusShift + massTol < peak.Mass) continue;
                    if (fragment.ExactMass - fragmentMinusShift - massTol > peak.Mass) break;

                    //Debug.WriteLine("Search mass pair\t" + peak.Mass + "\t" + fragments[i].SaturatedExactMass);
            ...
            foreach (var peakFragmentPair in peakFragmentPairs)
            {
                ...
                if (matchedInfo.FragmentID >= 0)
                {
                    ...
                    var massDiff = Math.Abs(peak.Mass - matchedInfo.MatchedMass);
                    var ppm = MolecularFormulaUtility.PpmCalculator(peak.Mass, matchedInfo.MatchedMass);
                ...
                { //it means the result is coming from Fragment DB 
                    //as it is
                }
        ...
        /// /// </summary>
        private static void doFragmentDbBasedSpectraAssignment(Structure structure, List<SpectrumPeak> peakList,
            ...
            for (int i = 0; i < peakList.Count; i++)
            {
                ...
                for (int j = 0; j < fragmentDB.Count; j++)
                {
                    ...
                    if (Math.Abs(peakMass - fragment.FragmentMass) < ms2Tol && isFormulaMatch(originalFormula, fragment.FragmentFormula))
                    {
                        ...
                        {
                            maxID = j;
                            maxScore = (massScore + jaccardScore) * 0.5; // Hirata 20230817
                        }
                    }
                }
                ...
                if (maxID >= 0)
                {
                    ...
                    var matchedInfo = new MatchedFragmentInfo()
                    {
                        ...
                        Ppm = Math.Round((peak.Mass - matchedFrag.FragmentMass) / matchedFrag.FragmentMass * 1000000, 4),
                        Massdiff = Math.Round(peak.Mass - matchedFrag.FragmentMass, 7),
                        IsEeRule = true,
                        IsHrRule = true,
                        IsSolventAdductFragment = false,
                        AssignedAdductMass = 0,
                        AssignedAdductString = string.Empty,
        ...
        private static List<PeakFragmentPair> getRefinedPeakFragmentPairs(List<PeakFragmentPair> peakFragmentPairs)
        {
            ...
            else return rPeakFragmentPairs.OrderBy(n => n.Peak.Mass).ToList();
        }

        /// <summary>
        /// Return MatchedFragmentInfo. If there is no match pair with any hydrogen shift, return null
        /// </summary>
        private static MatchedFragmentInfo getMatchedFragmentInfo(int fragmentID, Fragment fragment, SpectrumPeak peak,
            ...
            List<Fragment> fragments, List<PeakFragmentPair> peakFragmentPairs)
        {
            ...
            for (int i = initialCounter; i <= 2 * fragment.CleavedBondDictionary.Count + lastCounter; i++)
            {
                ...
                //if (Math.Abs(108.0499 - peak.Mass) < 0.05) {
                //    Console.WriteLine();
                //}
                ...
                //    Console.WriteLine();
                //}
                //Debug.WriteLine(peak.Mass + "\t" + matchedMass + "\t" + matchedMassWithAdduct + "\t" + Math.Abs(matchedMass - peak.Mass) + "\t" + Math.Abs(matchedMassWithAdduct - peak.Mass));

                if (matchedMass - massTol < peak.Mass && peak.Mass < matchedMass + massTol)
                {
                ...
                else if (matchedMassWithAdduct - massTol < peak.Mass && peak.Mass < matchedMassWithAdduct + massTol)
                {

                    setMatchedFragmentInfo(matchedFragmentInfo, fragment, peak, i, matchedMassWithAdduct, true, massTol, ionMode, solventAdduct,
                        AdductIonParser.GetAdductContent(adductString), fragments, peakFragmentPairs);
                    return matchedFragmentInfo;
                }
            }
        ...
        /// </summary>
        private static void setMatchedFragmentInfo(MatchedFragmentInfo matchedFragmentInfo, Fragment fragment, SpectrumPeak peak, int rearrangedHydrogen,
            ...
            List<Fragment> fragments, List<PeakFragmentPair> peakFragmentPairs)
        {
            ...
            matchedFragmentInfo.MaLikelihood = getGaussianSimilarity(peak.Mass, matchedMass, massTol);
        ...
        private static float getFragmentLikelihood(Fragment fragment, SpectrumPeak peak, double matchedMass, int rearrangedHydrogen, bool isSolventAdduct, IonMode ionMode, out bool isEeRule, out bool isHrRule)
        {
            ...
            if (totalAtomCount == fragment.CleavedAtomDictionary.Count)
            { // if other elements contained, the current program cannot evaluate the probability
