...
namespace CompMs.Common.Lipidomics
{
    public class EtherPCSpectrumGenerator : ILipidSpectrumGenerator
    {
        ...
        public IMSScanProperty Generate(Lipid lipid, AdductIon adduct, IMoleculeProperty molecule = null)
        {
            ...
            {
                spectrum.AddRange(GetSn1PositionSpectrum(lipid, plChains.GetAllChains()[0], adduct));
                AlkylChain alkyl;
                AcylChain acyl;
                ...
                {
                    spectrum.AddRange(GetEtherPCPSpectrum(lipid, alkyl, plChains.GetAllChains()[1], adduct));
                }
                else
                {
                    spectrum.AddRange(GetEtherPCOSpectrum(lipid, plChains.GetAllChains()[0], plChains.GetAllChains()[1], adduct));
                }
        ...
        private SpectrumPeak[] GetEtherPCSpectrum(ILipid lipid, AdductIon adduct)
        {
            var spectrum = new List<SpectrumPeak>
            {
                ...
                new SpectrumPeak(adduct.ConvertToMz(lipid.Mass)/2, 40d, "Precursor2+") { SpectrumComment = SpectrumComment.precursor },
        ...
        private SpectrumPeak[] GetEtherPCPSpectrum(ILipid lipid, IChain alkylChain, IChain acylChain, AdductIon adduct)
        {
            return new[]
            {
                new SpectrumPeak(adduct.ConvertToMz(lipid.Mass - alkylChain.Mass + MassDiffDictionary.HydrogenMass), 100d, $"-{alkylChain}") { SpectrumComment = SpectrumComment.acylchain },
                ...
                new SpectrumPeak(adduct.ConvertToMz(lipid.Mass - alkylChain.Mass - MassDiffDictionary.OxygenMass), 300, $"-{alkylChain}-O") { SpectrumComment = SpectrumComment.acylchain, IsAbsolutelyRequiredFragmentForAnnotation = true },
