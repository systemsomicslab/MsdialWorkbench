...
{
    public class LPCEidSpectrumGenerator : ILipidSpectrumGenerator
    {
        ...
        public LPCEidSpectrumGenerator() {
            spectrumGenerator = new SpectrumPeakGenerator();
        }

        public LPCEidSpectrumGenerator(ISpectrumPeakGenerator peakGenerator) {
        ...
        public IMSScanProperty Generate(Lipid lipid, AdductIon adduct, IMoleculeProperty molecule = null) {
            ...
            if (lipid.Chains is PositionLevelChains plChains)
            {
                ...
                spectrum.AddRange(GetAcylDoubleBondSpectrum(lipid, plChains.GetAllChains().OfType<AcylChain>(), adduct));
            }
            if (lipid.Chains is MolecularSpeciesLevelChains mlChains)
            {
                ...
                spectrum.AddRange(EidSpecificSpectrum(lipid, adduct, 0d, 250d));
                spectrum.AddRange(GetAcylDoubleBondSpectrum(lipid, mlChains.GetAllChains().OfType<AcylChain>(), adduct));
            }
        ...
        private SpectrumPeak[] GetLPCSpectrum(ILipid lipid, AdductIon adduct) {
            ...
            if (adduct.AdductIonName == "[M+H]+")
            {
                ...
                (
                     new[]
                     {
                         new SpectrumPeak(adduct.ConvertToMz(lipid.Mass - H2O), 200d, "C5H14NO+") { SpectrumComment = SpectrumComment.metaboliteclass },
        ...
        private SpectrumPeak[] GetAcylPositionSpectrum(ILipid lipid, IChain acylChain, AdductIon adduct) {
            return new[]
            {
                new SpectrumPeak(adduct.ConvertToMz(lipid.Mass- acylChain.Mass- MassDiffDictionary.OxygenMass - CH2 - MassDiffDictionary.HydrogenMass), 100d, "-CH2(Sn1)") { SpectrumComment = SpectrumComment.snposition },
            };
        }
