...
{
    public class PCEidSpectrumGenerator : ILipidSpectrumGenerator
    {
        ...
        public PCEidSpectrumGenerator()
        {
            spectrumGenerator = new SpectrumPeakGenerator();
        }

        public PCEidSpectrumGenerator(ISpectrumPeakGenerator spectrumGenerator)
        {
        ...
        public IMSScanProperty Generate(Lipid lipid, AdductIon adduct, IMoleculeProperty molecule = null)
        {
            ...
            if (lipid.Chains is MolecularSpeciesLevelChains mlChains)
            {
                ...
                spectrum.AddRange(GetAcylDoubleBondSpectrum(lipid, mlChains.GetAllChains().OfType<AcylChain>().Where(c => c.DoubleBond.UnDecidedCount == 0 && c.Oxidized.UnDecidedCount == 0), adduct));
            }
            if (lipid.Chains is PositionLevelChains plChains)
            {
                ...
                spectrum.AddRange(EidSpecificSpectrum(lipid, adduct, 0d, 200d));
                spectrum.AddRange(GetAcylDoubleBondSpectrum(lipid, plChains.GetAllChains().OfType<AcylChain>().Where(c => c.DoubleBond.UnDecidedCount == 0 && c.Oxidized.UnDecidedCount == 0), adduct));
            }
        ...
        private SpectrumPeak[] GetPCSpectrum(ILipid lipid, AdductIon adduct)
        {
            var spectrum = new List<SpectrumPeak>
            {
                ...
                new SpectrumPeak(adduct.ConvertToMz(Gly_C), 50d, "Gly-C")  { SpectrumComment = SpectrumComment.metaboliteclass },
                new SpectrumPeak(adduct.ConvertToMz(Gly_O), 50d, "Gly-O") { SpectrumComment = SpectrumComment.metaboliteclass },
