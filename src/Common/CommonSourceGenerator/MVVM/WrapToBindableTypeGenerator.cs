...
""");
        ...
        private static void Emit(SourceProductionContext context, GeneratorAttributeSyntaxContext source) {
            ...
            IEnumerable<IPropertySymbol> publicGetterProperties = namedTypeSymbol.GetMembers()
            ...
            ToGetterOnlyProperty(sb, publicGetterProperties, out var cares);

            IEnumerable<IPropertySymbol> publicProperties = namedTypeSymbol.GetMembers()
                ...
                .Cast<IPropertySymbol>();
            ToBindableProperty(sb, publicProperties, cares);
...
""";
        ...
        private static void ToBindableProperty(StringBuilder sb, IEnumerable<IPropertySymbol> properties, Dictionary<string, HashSet<string>> cares) {
...
""");
        ...
        private static void ToGetterOnlyProperty(StringBuilder sb, IEnumerable<IPropertySymbol> properties, out Dictionary<string, HashSet<string>> cares) {
            ...
            foreach (var property in properties) {
                ...
                if (getterSyntax is not null) {
                    sb.AppendLine($$"""
    /*
        GetSyntax: {{getterSyntax.ToFullString()}}
        SyntaxNode type: {{getterSyntax.GetType()}}
    */
""");
                    foreach (var node in getterSyntax.DescendantNodes().OfType<IdentifierNameSyntax>()) {
                        sb.AppendLine($$"""
    /*
        GetSyntax: {{node.ToFullString()}}
        SyntaxNode type: {{node.GetType()}}
    */
""");
...
""");
        ...
        class PropertyUsageCollector : CSharpSyntaxWalker {
            public List<string> UsedProperties { get; } = new();

            public override void VisitMemberAccessExpression(MemberAccessExpressionSyntax node) {
                var symbol = node.Name;
                if (symbol is IdentifierNameSyntax identifierNameSyntax) {
                    UsedProperties.Add(identifierNameSyntax.Identifier.Text);
                }
