...
public enum VerticalTickPlacement {
    None,
    Left,
    Right,
}
...
public class VerticalAxisControl : ChartBaseControl
{
    ...
    public static readonly DependencyProperty TickPlacementProperty = DependencyProperty.Register(
        nameof(TickPlacement), typeof(VerticalTickPlacement), typeof(VerticalAxisControl),
        new FrameworkPropertyMetadata(VerticalTickPlacement.Right, FrameworkPropertyMetadataOptions.AffectsRender)
        );
    ...
    public VerticalTickPlacement TickPlacement {
        get => (VerticalTickPlacement)GetValue(TickPlacementProperty);
    ...
    protected override void Update() {
        ...
        double horizontalBasePoint = TickPlacement switch
        {
            VerticalTickPlacement.Left => 0d,
            _ => actualWidth,
        };
        ...
        foreach (var data in labelTicks) {
            ...
            switch (data.TickType) {
                case TickType.LongTick:
                    dc.DrawLine(TickPen, new Point(horizontalBasePoint, center), new Point(Math.Abs(horizontalBasePoint - LongTickSize), center));
                    ...
                    dc.DrawText(formattedText, new Point(Math.Abs(Math.Max(0d, horizontalBasePoint - formattedText.Width) - LongTickSize), center - formattedText.Height / 2));
                    break;
                case TickType.ShortTick:
                    dc.DrawLine(TickPen, new Point(horizontalBasePoint, center), new Point(Math.Abs(horizontalBasePoint - ShortTickSize), center));
                    break;
            }
            dc.Close();
