...
namespace Riken.Metabolomics.Lipidomics.Searcher
{
    public sealed class LipidMsmsCharacterization
    {
        ...
        public static LipidMolecule JudgeIfPhosphatidylThreonine(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
           AdductIon adduct)
        {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            var query = new List<Peak> {
                                ...
                                new Peak() { Mz = nl_SN2, Intensity = 0.01 },
                                new Peak() { Mz = nl_NS2_H2O, Intensity = 0.01 },
                                new Peak() { Mz = nl_HD_SN1, Intensity = 0.01 },
                                new Peak() { Mz = nl_HD_SN2, Intensity = 0.01 }
                            };
                            ...
                            if (foundCount >= 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("PT", LbmClass.PT, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("PT", LbmClass.PT, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            else
    ...
    AdductIon adduct, int TotalOxidized, int sn1MaxOxidized, int sn2MaxOxidized)
        ...
        public static LipidMolecule JudgeIfMgmg(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    // seek SN1+
                    var threshold1 = 1;
                    var diagnosticMz1 = acylCainMass(totalCarbon, totalDoubleBond) - Electron;
                    var isClassIon1Found = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz1, threshold1);
                    if (isClassIon1Found == false) return null;
                    ...
                    return returnAnnotationResult("MGMG", LbmClass.MGMG, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
            }
            else
        ...
        public static LipidMolecule JudgeIfDgmg(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    // seek SN1(acyl)+
                    var diagnosticMz2 = acylCainMass(totalCarbon, totalDoubleBond) - Electron;

                    var isClassIon1Found = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz1, threshold);
                    var isClassIon2Found = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz2, threshold);
                    if (isClassIon1Found == false || isClassIon2Found == false) return null;
                    return returnAnnotationResult("DGMG", LbmClass.DGMG, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
            }
            else
                                ...
                                for (int sn1Carbon = minSn1Carbon; sn1Carbon <= carbonLimit2; sn1Carbon++)
                                {
                                    for (int sn1Double = minSn1DoubleBond; sn1Double <= doubleLimit2; sn1Double++)
                                    {
                                        ...
                                        if (foundCount >= 3)
                                        {
                                            //averageIntensity = averageIntensity + averageIntensity2;
                                            if (averageIntensity > 100) averageIntensity = 100;
                                            var molecule = getCardiolipinMoleculeObjAsLevel2_1("CL", LbmClass.CL, sn1Carbon, sn2Carbon,
                                                           sn3Carbon, sn4Carbon, sn1Double, sn2Double, sn3Double, sn4Double, averageIntensity);
                                            candidates.Add(molecule);
                                        }
                                    }
        ...
        public static LipidMolecule JudgeIfDmedFattyacid(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond,
           int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
           AdductIon adduct)
        {
        ...
        public static LipidMolecule JudgeIfDmedOxfattyacid(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond,
              int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
                 AdductIon adduct, int totalOxidized)
        {
                ...
                else if (adductform == "[M+Na]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            var sph1 = SphingoChainMass(sphCarbon, sphDouble) + MassDiffDictionary.HydrogenMass - MassDiffDictionary.OxygenMass;
                ...
                if (adductform == "[M+H]+" || adductform == "[M+H-H2O]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++) // sphingo chain must have double bond
                        {
                            ...
                            var query = new List<Peak> {
                                new Peak() { Mz = sph1, Intensity = 5 },
                                new Peak() { Mz = sph2, Intensity = 10 },
                                new Peak() { Mz = sph3, Intensity = 5 },
                ...
                if (adductform == "[M+H]+" || adductform == "[M+H-H2O]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++) // sphingo chain must have double bond
                        {
                            ...
                            var query = new List<Peak> {
                                new Peak() { Mz = sph1, Intensity = 3 },
                                new Peak() { Mz = sph2, Intensity = 3 },
                                new Peak() { Mz = sph3, Intensity = 3 },
                                //new Peak() { Mz = acylamide, Intensity = 0.01 }
                            };
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            var query = new List<Peak> {
                                new Peak() { Mz = sph1, Intensity = 5 },
                                new Peak() { Mz = sph2, Intensity = 5 },
                                new Peak() { Mz = sph3, Intensity = 10 },
                                new Peak() { Mz = sph4, Intensity = 5 },
                                //new Peak() { Mz = acylamide, Intensity = 0.01 }
                            };
        ...
        public static LipidMolecule JudgeIfGD1a(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+2H]2+" || adduct.AdductIonName == "[M+2NH4]2+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("GD1a", LbmClass.GD1a, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
        ...
        public static LipidMolecule JudgeIfGD1b(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
    AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+2H]2+" || adduct.AdductIonName == "[M+2NH4]2+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("GD1b", LbmClass.GD1b, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    ...
                    return returnAnnotationResult("GD1b", LbmClass.GD1b, "d", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
        ...
        public static LipidMolecule JudgeIfGD2(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
            ...
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
            {
                ...
                return returnAnnotationResult("GD2", LbmClass.GD2, "d", theoreticalMz, adduct,
                    totalCarbon, totalDoubleBond, 0, candidates, 2);
            }
            return null;
        }
        public static LipidMolecule JudgeIfGD3(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
            ...
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
            {
                ...
                return returnAnnotationResult("GD3", LbmClass.GD3, "d", theoreticalMz, adduct,
                    totalCarbon, totalDoubleBond, 0, candidates, 2);
            }
            return null;
        }

        public static LipidMolecule JudgeIfGM1(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+2H]2+" || adduct.AdductIonName == "[M+2NH4]2+" || adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("GM1", LbmClass.GM1, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
        ...
        public static LipidMolecule JudgeIfGT1b(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
    AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+2H]2+" || adduct.AdductIonName == "[M+2NH4]2+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("GT1b", LbmClass.GT1b, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    ...
                    return returnAnnotationResult("GT1b", LbmClass.GT1b, "d", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
        ...
        public static LipidMolecule JudgeIfGQ1b(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
    AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+2H]2+" || adduct.AdductIonName == "[M+2NH4]2+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("GQ1b", LbmClass.GQ1b, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("GQ1b", LbmClass.GQ1b, "d", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
        ...
        public static LipidMolecule JudgeIfNGcGM3(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
    AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+NH4]+" || adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            if (foundCount >= 1)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("NGcGM3", LbmClass.NGcGM3, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("NGcGM3", LbmClass.NGcGM3, "d", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            if (adduct.AdductIonName == "[M-H]-" || adduct.AdductIonName == "[M-2H]2-")
    ...
    int totalCarbon, int totalDoubleBond, AdductIon adduct, int totalOxidized)
        ...
        public static LipidMolecule JudgeIfGpnae(ObservableCollection<double[]> spectrum, double ms2Tolerance, float theoreticalMz,
            int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
            { // Positive ion mode 
                ...
                {
                    var threshold = 10;
                    var diagnosticMz = acylCainMass(totalCarbon, totalDoubleBond) + 12 * 2 + MassDiffDictionary.HydrogenMass * 4 + MassDiffDictionary.NitrogenMass + Proton;
                    var isClassIonFound = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz, threshold);
                    if (isClassIonFound != true)
                    {
                        return null;
    ...
    AdductIon adduct)
        ...
        public static LipidMolecule JudgeIfVitaminEmolecules(ObservableCollection<double[]> spectrum, double ms2Tolerance,
                double theoreticalMz, int totalCarbon, int totalDoubleBond, // 
                AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
        ...
        public static LipidMolecule JudgeIfVitaminDmolecules(ObservableCollection<double[]> spectrum, double ms2Tolerance,
        double theoreticalMz, int totalCarbon, int totalDoubleBond, // 
        AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+H]+" || adduct.AdductIonName == "[M+Na]+")
                {
                    ...
                    if (isVitamindFound == true)
                    {
                        ...
                        //
                        var candidates = new List<LipidMolecule>();
                        return returnAnnotationNoChainResult("25-hydroxycholecalciferol", LbmClass.Vitamin_D, "", theoreticalMz, adduct,
                           totalCarbon, totalDoubleBond, 0, candidates, 0);
                    }
                }
            }
    ...
    double theoreticalMz, int totalCarbon, int totalDoubleBond, AdductIon adduct)
     ...
     AdductIon adduct)
        ...
        public static LipidMolecule JudgeIfErgoSESpecies(ObservableCollection<double[]> spectrum, double ms2Tolerance, float theoreticalMz,
        int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfDesmosterolSpecies(ObservableCollection<double[]> spectrum, double ms2Tolerance, float theoreticalMz,
            int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
   ...
   double theoreticalMz, int totalCarbon, int totalDoubleBond,
    ...
    AdductIon adduct)
        ...
        public static LipidMolecule JudgeIfnoChainSterol(string lipidname, LbmClass lipidclass, ObservableCollection<double[]> spectrum, double ms2Tolerance,
       double theoreticalMz, int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfMonoMethylPE(ObservableCollection<double[]> spectrum, double ms2Tolerance,
        double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
        int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
        AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount == 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("MMPE", LbmClass.MMPE, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("MMPE", LbmClass.MMPE, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            else
                ...
                if (adduct.AdductIonName == "[M-H]-")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            var query = new List<Peak> {
                            new Peak() { Mz = sn1, Intensity = 10.0 },
                            new Peak() { Mz = sn2, Intensity = 10.0 }
                        };
        ...
        public static LipidMolecule JudgeIfDiMethylPE(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount == 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("DMPE", LbmClass.DMPE, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("DMPE", LbmClass.DMPE, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
            }
            else
        ...
        public static LipidMolecule JudgeIfMipc(ObservableCollection<double[]> spectrum, double ms2Tolerance,
        double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
        int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            var query = new List<Peak> {
                            new Peak() { Mz = sph1, Intensity = 0.1 },
                            new Peak() { Mz = sph2, Intensity = 0.1 }
                        };
        ...
        //add 20200812
        public static LipidMolecule JudgeIfOxTriacylglycerol(ObservableCollection<double[]> spectrum, double ms2Tolerance,
               double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
               int minSn1Carbon, int maxSn1Carbon, int minSn1DoubleBond, int maxSn1DoubleBond,
               int minSn2Carbon, int maxSn2Carbon, int minSn2DoubleBond, int maxSn2DoubleBond,
               int totalOxidized, AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                        ...
                        for (int sn1Double = minSn1DoubleBond; sn1Double <= maxSn1DoubleBond; sn1Double++)
                        {
                            ...
                            for (int sn2Carbon = minSn2Carbon; sn2Carbon <= carbonLimit; sn2Carbon++)
                            {
                                for (int sn2Double = minSn2DoubleBond; sn2Double <= doubleLimit; sn2Double++)
                                {
                                    ...
                                    var nl_SN1 = diagnosticMz - acylCainMass(sn1Carbon, sn1Double) - H2O + MassDiffDictionary.HydrogenMass;
                                    var nl_SN2 = diagnosticMz - acylCainMass(sn2Carbon, sn2Double) - H2O + MassDiffDictionary.HydrogenMass;
                                    var nl_SN3 = diagnosticMz - acylCainMass(sn3Carbon, sn3Double) - H2O + MassDiffDictionary.HydrogenMass - (MassDiffDictionary.OxygenMass * totalOxidized);
                                    ...
                                    var query = new List<Peak> {
                                        ...
                                        new Peak() { Mz = nl_SN2, Intensity = 5 },
                                        new Peak() { Mz = nl_SN3, Intensity = 5 },
                                        new Peak() { Mz = nl_SN1andH2O, Intensity = 5 },
                                        new Peak() { Mz = nl_SN2andH2O, Intensity = 5 }
                                    };
                                    ...
                                    { // 3 chains must be observed.
                                        var molecule = getOxTriacylglycerolMoleculeObjAsLevel2("TG", LbmClass.OxTG, sn1Carbon, sn1Double,
                    ...
                    if (candidates == null || candidates.Count == 0) return null;
                    return returnAnnotationResult("TG", LbmClass.OxTG, string.Empty, theoreticalMz, adduct,
                                ...
                                for (int sn2Double = minSn2DoubleBond; sn2Double <= doubleLimit; sn2Double++)
                                {
                                    ...
                                    for (int sn3Carbon = minSn3Carbon; sn3Carbon <= carbonLimit2; sn3Carbon++)
                                    {
                                        for (int sn3Double = minSn3DoubleBond; sn3Double <= doubleLimit2; sn3Double++)
                                        {
                                            ...
                                            var nl_SN1 = diagnosticMz - acylCainMass(sn1Carbon, sn1Double) - H2O + MassDiffDictionary.HydrogenMass;
                                            var nl_SN2 = diagnosticMz - acylCainMass(sn2Carbon, sn2Double) - H2O + MassDiffDictionary.HydrogenMass;
                                            var nl_SN4 = diagnosticMz - acylCainMass(sn4Carbon, sn4Double) - H2O + MassDiffDictionary.HydrogenMass;
                                            var nl_SN3andSN4 = nl_SN4 - acylCainMass(sn3Carbon, sn3Double) - H2O + MassDiffDictionary.HydrogenMass * 3;
                                            var nl_SN1andSN4 = nl_SN4 - acylCainMass(sn1Carbon, sn1Double) - H2O + MassDiffDictionary.HydrogenMass;
                                            var nl_SN2andSN4 = nl_SN4 - acylCainMass(sn2Carbon, sn2Double) - H2O + MassDiffDictionary.HydrogenMass;

                                            var query = new List<Peak>
                                            {
                                                new Peak() { Mz = nl_SN1, Intensity = 5 },
                                                new Peak() { Mz = nl_SN2, Intensity = 5 },
                                                new Peak() { Mz = nl_SN4, Intensity = 5 },
                                                new Peak() { Mz = nl_SN3andSN4, Intensity = 5 },
                                                new Peak() { Mz = nl_SN1andSN4, Intensity = 5 },
                                                new Peak() { Mz = nl_SN2andSN4, Intensity = 5 }
                                            };
        ...
        // add20200818
        public static LipidMolecule JudgeIfNAcylGlySerOxFa(ObservableCollection<double[]> spectrum, double ms2Tolerance,
        ...
        public static LipidMolecule JudgeIfNAcylGlyOxFa(ObservableCollection<double[]> spectrum, double ms2Tolerance,
               double theoreticalMz, int totalCarbon, int totalDoubleBond, int totalOxidized,
               AdductIon adduct)  //NAcylGlyFa as same
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
            { // Positive ion mode 
        ...
        public static LipidMolecule JudgeIfNAcylOrnOxFa(ObservableCollection<double[]> spectrum, double ms2Tolerance,
       double theoreticalMz, int totalCarbon, int totalDoubleBond, int totalOxidized,
       AdductIon adduct)//NAcylOrnFa as same
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
            { // Positive ion mode 
        ...
        // add 20211026
        public static LipidMolecule JudgeIfSteroidWithLpa(string lipidname, LbmClass lipidclass, ObservableCollection<double[]> spectrum, double ms2Tolerance,
          double theoreticalMz, int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
        ...
        public static LipidMolecule JudgeIfSteroidWithPa(string lipidname, LbmClass lipidclass, ObservableCollection<double[]> spectrum, double ms2Tolerance,
                  double theoreticalMz, int totalCarbon, int totalDoubleBond,
                  int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
                  AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M-H]-")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            var query = new List<Peak> {
                                new Peak() { Mz = sn1Fa, Intensity = 0.1 },
                                new Peak() { Mz = sn2Fa, Intensity = 0.1 },
                            };
        ...
        // add 20220201
        public static LipidMolecule JudgeIfSpeSpecies(string lipidname, LbmClass lipidclass, ObservableCollection<double[]> spectrum, double ms2Tolerance,
       double theoreticalMz, int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfNAcylTauFa(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, int totalOxidized,
            AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
        ...
        public static LipidMolecule JudgeIfNAcylPheFa(ObservableCollection<double[]> spectrum, double ms2Tolerance,
         double theoreticalMz, int totalCarbon, int totalDoubleBond, int totalOxidized,
         AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
        ...
        //add 20230407
        public static LipidMolecule JudgeIfPhosphatidylcholineD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
    AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount >= 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("PC_d5", LbmClass.PC_d5, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("PC_d5", LbmClass.PC_d5, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
        ...
        public static LipidMolecule JudgeIfPhosphatidylethanolamineD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount == 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("PE_d5", LbmClass.PE_d5, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("PE_d5", LbmClass.PE_d5, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
                //addMT
                else if (adduct.AdductIonName == "[M+Na]+")
        ...
        public static LipidMolecule JudgeIfPhosphatidylserineD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
           AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount >= 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("PS_d5", LbmClass.PS_d5, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("PS_d5", LbmClass.PS_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 2);
        ...
        public static LipidMolecule JudgeIfPhosphatidylglycerolD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
           AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount == 2 && averageIntensity < 30)
                            { // average intensity < 30 is nessesarry to distinguish it from BMP
                                var molecule = getPhospholipidMoleculeObjAsLevel2("PG_d5", LbmClass.PG_d5, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("PG_d5", LbmClass.PG_d5, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
                else if (adduct.AdductIonName == "[M+Na]+")
                {
        ...
        public static LipidMolecule JudgeIfPhosphatidylinositolD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
           AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    return returnAnnotationResult("PI_d5", LbmClass.PI_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
                else if (adduct.AdductIonName == "[M+Na]+")
                {
        ...
        public static LipidMolecule JudgeIfLysopcD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // 
    int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
    AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    var score = 0.0;
                    if (totalCarbon < 30) score = score + 1.0;
                    var molecule = getSingleacylchainwithsuffixMoleculeObjAsLevel2("LPC_d5", LbmClass.LPC_d5, totalCarbon, totalDoubleBond,
                    score, chainSuffix);
                    candidates.Add(molecule);

                    return returnAnnotationResult("LPC_d5", LbmClass.LPC_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 1);
        ...
        public static LipidMolecule JudgeIfLysopeD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // 
            int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
            AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+H]+")
                {
                    ...
                    var candidates = new List<LipidMolecule>();
                        return returnAnnotationResult("LPE_d5", LbmClass.LPE_d5, "", theoreticalMz, adduct,
                           totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
                else if (adduct.AdductIonName == "[M+Na]+")
                {
        ...
        public static LipidMolecule JudgeIfLysopgD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
    double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
    int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
    AdductIon adduct)
        {
            ...
            { //negative ion mode only
                if (adduct.AdductIonName == "[M-H]-")
                {
                    ...
                    //averageIntensity);
                    //candidates.Add(molecule);
                    return returnAnnotationResult("LPG_d5", LbmClass.LPG_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
            }
            else if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfLysopiD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
            AdductIon adduct)
        {
            ...
            { //negative ion mode only
                if (adduct.AdductIonName == "[M-H]-")
                {
                    ...
                    //averageIntensity);
                    //candidates.Add(molecule);
                    return returnAnnotationResult("LPI_d5", LbmClass.LPI_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
            }
            else if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfLysopsD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PE 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
            AdductIon adduct)
        {
            ...
            { //negative ion mode only
                if (adduct.AdductIonName == "[M-H]-")
                {
                    ...
                    //averageIntensity);
                    //candidates.Add(molecule);
                    return returnAnnotationResult("LPS_d5", LbmClass.LPS_d5, "", theoreticalMz, adduct,
                       totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
            }
            else if (adduct.IonMode == IonMode.Positive)
        ...
        public static LipidMolecule JudgeIfDagD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSn1Carbon, int maxSn1Carbon, int minSn1DoubleBond, int maxSn1DoubleBond,
           AdductIon adduct)
        {
                ...
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    for (int sn1Carbon = minSn1Carbon; sn1Carbon <= maxSn1Carbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSn1DoubleBond; sn1Double <= maxSn1DoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount == 2)
                            {
                                var molecule = getDiacylglycerolMoleculeObjAsLevel2("DG_d5", LbmClass.DG_d5, sn1Carbon, sn1Double,
                                sn2Carbon, sn2Double,
                                averageIntensity);
                                candidates.Add(molecule);
                            }
                    ...
                    return returnAnnotationResult("DG_d5", LbmClass.DG_d5, string.Empty, theoreticalMz, adduct,
        ...
        public static LipidMolecule JudgeIfTriacylglycerolD5(ObservableCollection<double[]> spectrum, double ms2Tolerance,
           double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PS 46:6, totalCarbon = 46 and totalDoubleBond = 6
           int minSn1Carbon, int maxSn1Carbon, int minSn1DoubleBond, int maxSn1DoubleBond,
           int minSn2Carbon, int maxSn2Carbon, int minSn2DoubleBond, int maxSn2DoubleBond,
           AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                        ...
                        for (int sn1Double = minSn1DoubleBond; sn1Double <= maxSn1DoubleBond; sn1Double++)
                        {
                            ...
                            for (int sn2Carbon = minSn2Carbon; sn2Carbon <= carbonLimit; sn2Carbon++)
                            {
                                for (int sn2Double = minSn2DoubleBond; sn2Double <= doubleLimit; sn2Double++)
                                {
                                    ...
                                    if (foundCount == 3)
                                    { // these three chains must be observed.
                                        var molecule = getTriacylglycerolMoleculeObjAsLevel2("TG_d5", LbmClass.TG_d5, sn1Carbon, sn1Double,
                                            sn2Carbon, sn2Double, sn3Carbon, sn3Double, averageIntensity);
                                        candidates.Add(molecule);
                                    }
                                }
                    ...
                    if (candidates == null || candidates.Count == 0) return null;
                    return returnAnnotationResult("TG_d5", LbmClass.TG_d5, string.Empty, theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 3);
                }
                else if (adduct.AdductIonName == "[M+Na]+")
                {   //add MT
        ...
        public static LipidMolecule JudgeIfSphingomyelinD9(ObservableCollection<double[]> spectrum, double ms2Tolerance,
        double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
        int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
        AdductIon adduct)
            {
                    ...
                    var isClassIonFound = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz, threshold);
                        ...
                        for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                        {
                            for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                            {
                                ...
                                if (foundCount == 1)
                                { // the diagnostic acyl ion must be observed for level 2 annotation
                                    var molecule = getCeramideMoleculeObjAsLevel2("SM_d9", LbmClass.SM_d9, "d", sphCarbon, sphDouble,
                                        acylCarbon, acylDouble, averageIntensity);
                                    candidates.Add(molecule);
                                }
                            }
                        }
                        return returnAnnotationResult("SM_d9", LbmClass.SM_d9, "d", theoreticalMz, adduct,
                            totalCarbon, totalDoubleBond, 0, candidates, 2);
        ...
        public static LipidMolecule JudgeIfCeramidensD7(ObservableCollection<double[]> spectrum, double ms2Tolerance,
            double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
            int minSphCarbon, int maxSphCarbon, int minSphDoubleBond, int maxSphDoubleBond,
            AdductIon adduct)
        {
                ...
                if (adductform == "[M+H]+" || adductform == "[M+H-H2O]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++) // sphingo chain must have double bond
                        {
                            ...
                            if (foundCount >= foundCountThresh)
                            { // 
                                var molecule = getCeramideMoleculeObjAsLevel2("Cer_d7", LbmClass.Cer_NS_d7, "d", sphCarbon, sphDouble,
                                    acylCarbon, acylDouble, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    ...
                    return returnAnnotationResult("Cer_d7", LbmClass.Cer_NS_d7, "d", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
                else if (adductform == "[M+Na]+")
                {
                    ...
                    for (int sphCarbon = minSphCarbon; sphCarbon <= maxSphCarbon; sphCarbon++)
                    {
                        for (int sphDouble = minSphDoubleBond; sphDouble <= maxSphDoubleBond; sphDouble++)
                        {
                            ...
                            var sph1 = SphingoChainMass(sphCarbon, sphDouble) + MassDiffDictionary.HydrogenMass - MassDiffDictionary.OxygenMass+(MassDiffDictionary.HydrogenMass * 7);
                            var sph3 = sph1 - H2O + Proton;
        ...
        public static LipidMolecule JudgeIfCholesterylEsterD7(ObservableCollection<double[]> spectrum, double ms2Tolerance, float theoreticalMz,
        int totalCarbon, int totalDoubleBond, AdductIon adduct)
        {
            if (spectrum == null || spectrum.Count == 0) return null;
            if (adduct.IonMode == IonMode.Positive)
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+NH4]+")
                {
                    ...
                    return returnAnnotationResult("CE_d7", LbmClass.CE_d7, string.Empty, theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 1);
                }
                else if (adduct.AdductIonName == "[M+Na]+")
                {
        ...
        //20230424
        public static LipidMolecule JudgeIfBetaMethylPhosphatidylcholine(ObservableCollection<double[]> spectrum, double ms2Tolerance,
             double theoreticalMz, int totalCarbon, int totalDoubleBond, // If the candidate PC 46:6, totalCarbon = 46 and totalDoubleBond = 6
             int minSnCarbon, int maxSnCarbon, int minSnDoubleBond, int maxSnDoubleBond,
             AdductIon adduct)
        {
            ...
            { // positive ion mode 
                if (adduct.AdductIonName == "[M+H]+")
                {
                    // seek 198 (C6H17NO4P)
                    var threshold = 10.0;
                    var diagnosticMz = 12 * 6 + MassDiffDictionary.HydrogenMass * 16 + MassDiffDictionary.NitrogenMass + MassDiffDictionary.OxygenMass * 4 + MassDiffDictionary.PhosphorusMass + Proton;
                    var isClassIonFound = isDiagnosticFragmentExist(spectrum, ms2Tolerance, diagnosticMz, threshold);
                    if (isClassIonFound == false) return null;
                    // from here, acyl level annotation is executed.
                    var candidates = new List<LipidMolecule>();
                    for (int sn1Carbon = minSnCarbon; sn1Carbon <= maxSnCarbon; sn1Carbon++)
                    {
                        for (int sn1Double = minSnDoubleBond; sn1Double <= maxSnDoubleBond; sn1Double++)
                        {
                            ...
                            if (foundCount >= 2)
                            { // now I set 2 as the correct level
                                var molecule = getPhospholipidMoleculeObjAsLevel2("bmPC", LbmClass.bmPC, sn1Carbon, sn1Double,
                                    sn2Carbon, sn2Double, averageIntensity);
                                candidates.Add(molecule);
                            }
                        }
                    }
                    return returnAnnotationResult("bmPC", LbmClass.bmPC, "", theoreticalMz, adduct,
                        totalCarbon, totalDoubleBond, 0, candidates, 2);
                }
                //addMT
                else if (adduct.AdductIonName == "[M+Na]+")
   ...
   int sn1Carbon, int sn1Double, int sn2Carbon, int sn2Double, int sn3Carbon, int sn3Double, double score)
    ...
    string hydroxyString,
     int sphCarbon, int sphDouble, int acylCarbon, int acylDouble, int acylOxidized, double score)
        ...
        private static LipidMolecule getOxTriacylglycerolMoleculeObjAsLevel2(string lipidClass, LbmClass lbmClass,
        ...
        private static LipidMolecule getFahfaTriacylglycerolMoleculeObjAsLevel2(string lipidClass, LbmClass lbmClass,
