...
namespace edu.ucdavis.fiehnlab.msdial.Readers {
	public class GCDecReaderLegacy {
		public static List<MS1DecResult> ReadAll(FileStream fs) {
			...
			for (int i = 0; i < seekpointList.Count; i++) {
				...
				ms1DecResult.BasepeakHeight = BitConverter.ToSingle(buffer, 20);
				ms1DecResult.BasepeakArea = BitConverter.ToSingle(buffer, 24);
				...
				//Spectrum num
				int spectraNumber = BitConverter.ToInt32(buffer, 80);
				int datapointNumber = BitConverter.ToInt32(buffer, 84);
