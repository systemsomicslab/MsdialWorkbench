...
namespace Rfx.Riken.OsakaUniv
{
    ...
    public sealed class CowAlignment
    {
        ...
        /// <returns></returns>
        public static List<double[]> CorrelationOptimizedWarping(int minSlack, int maxSlack, int segmentSize, List<double[]> referenceChromatogram, List<double[]> sampleChromatogram, BorderLimit borderLimit)
        {
            List<double[]> alignedChromatogram = new List<double[]>();
            int referenceDatapointNumber = referenceChromatogram.Count, sampleDatapointNumber = sampleChromatogram.Count, massSpectraNumber = referenceChromatogram[0].Length - 3;
            ...
            Dictionary<int, bool> includeDictionary = new Dictionary<int, bool>();
            ...
            for (int i = 0; i < massSpectraNumber; i++) { includeDictionary[i] = true; }
            #endregion
            ...
            for (int i = segmentNumber - 1; i >= 0; i--)
            {
                ...
                for (int x = intervalStart; x <= intervalEnd; x++)
                {
                    for (int u = delta - slack[i]; u <= delta + slack[i]; u++)
                    {
                        ...
                        {
                            cumCoefficient = functionMatrixBean[i + 1, x + segmentSize + u].Score + cowFunctionCalculation(i, x, u, segmentSize, referenceChromatogram, sampleChromatogram, includeDictionary);
            ...
            //Backtrace
            #region
            double[] peakInformation = new double[4];
            ...
            for (int i = 0; i < segmentNumber; i++)
            {
                ...
                for (int j = 0; j < segmentSize + warp; j++)
                {
                    ...
                    //Set
                    peakInformation = new double[3 + massSpectraNumber];
                    peakInformation[0] = counter;
                    peakInformation[1] = referenceChromatogram[counter][1];
                    peakInformation[2] = (1 - fraction) * sampleChromatogram[positionFlont][0] + fraction * sampleChromatogram[positionEnd][0];
                    for (int k = 0; k < massSpectraNumber; k++) peakInformation[3 + k] = (1 - fraction) * sampleChromatogram[positionFlont][3 + k] + fraction * sampleChromatogram[positionEnd][3 + k];
            ...
            if (enabledLength < referenceDatapointNumber)
            {
                for (int i = enabledLength; i < referenceDatapointNumber; i++)
                {
                    ...
                    peakInformation[0] = counter;
                    peakInformation[1] = referenceChromatogram[counter][1];
                    peakInformation[2] = sampleChromatogram[positionEnd][0];
                    for (int k = 0; k < massSpectraNumber; k++) peakInformation[3 + k] = sampleChromatogram[positionEnd][3 + k];
                    alignedChromatogram.Add(peakInformation);
                    counter++;
                }
            }
        ...
        private static double cowFunctionCalculation(int rowPosition, int columnPosition, int u, int segmentSize, List<double[]> referenceChromatogram, List<double[]> sampleChromatogram, Dictionary<int, bool> includeDictionary)
        {
            int positionFlont, positionEnd, massSpectraNumber = referenceChromatogram[0].Length - 3, counter = 0;
            double warpedPosition, fraction, wT, wS, coefficient = 0;
            double[] targetArray = new double[segmentSize + u + 1], alingedArray = new double[segmentSize + u + 1];

            for (int i = 0; i < massSpectraNumber; i++)
            {
                if (includeDictionary[i] == false) continue;
                for (int j = 0; j <= segmentSize + u; j++)
                {
                    ...
                    wT = referenceChromatogram[columnPosition + j][3 + i];
                    wS = (1 - fraction) * sampleChromatogram[positionFlont][3 + i] + fraction * sampleChromatogram[positionEnd][3 + i];

                    targetArray[j] = wT;
                    alingedArray[j] = wS;
                }
                coefficient += Math.Abs(BasicMathematics.Coefficient(targetArray, alingedArray)); counter++;
            }
            return coefficient = coefficient / counter;
        }
