...
namespace CompMs.MsdialLcMsApi.Algorithm
{
    public sealed class PeakSpotting {
        ...
        public Task<List<ChromatogramPeakFeature>> RunAsync(IDataProvider provider, MsdialLcmsParameter param, IProgress<int>? progress, CancellationToken token = default) {
            ...
            return coreProcess.Execute3DFeatureDetectionAsync(_analysisFile, provider, chromatogramRange, param.NumThreads == 1 ? 1 : 2, ReportProgress.FromRange(progress, _initialProgress, _progressMax), token);
        }
    }
}
