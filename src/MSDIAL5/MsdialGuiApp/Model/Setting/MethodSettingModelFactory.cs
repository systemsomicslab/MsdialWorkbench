            ...
            var factory = new FacadeDataProviderFactory(file => {
                if (file.AnalysisFilePath.EndsWith(".wiff2")) {
                    lock (_wiff2FactoryLock) {
                        return new Wiff2DataProviderFactory() { NumThreads = 8, PeakTopSearchRange = storage.Parameter.ChromDecBaseParam.Q1DeconvolutionMs2AcquisitionRadius, }.ContraMap((AnalysisFileBean file) => file.AnalysisFilePath).CacheMS1();
                    }
                }
