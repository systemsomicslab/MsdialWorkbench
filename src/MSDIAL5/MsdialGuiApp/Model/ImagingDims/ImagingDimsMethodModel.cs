...
internal sealed class ImagingDimsMethodModel : MethodModelBase, IMethodModel
{
        ...
        : base(analysisFileBeanModelCollection, alignmentFileBeanModelCollection, projectBaseParameter)
    {
        ...
        _providerFactory = new StandardDataProviderFactory() { IsGuiProcess = true, IsImaging = true, Retry = 5, SleepTimeBeforeRetry = 5000 }.ContraMap((AnalysisFileBean file) => file.AnalysisFilePath);
        ImageModels = [];
    ...
    public AnalysisResultExportModel CreateExportAnalysisModel() {
        ...
        var models = new IMsdialAnalysisExport[]
        {
            ...
            new SpectraTypeSelectableMsdialAnalysisExportModel(new Dictionary<ExportspectraType, IAnalysisExporter<ChromatogramPeakFeatureCollection>> {
                ...
                [ExportspectraType.centroid] = new AnalysisMspExporter(_storage.DataBaseMapper, _storage.Parameter, file => new CentroidMsScanPropertyLoader(_storage.Parameter.ProviderFactoryParameter.Create(_providerFactory).Create(file), _storage.Parameter.MS2DataType)),
            })
            {
                FilePrefix = "Msp",
                FileSuffix = "msp",
            ...
            new SpectraTypeSelectableMsdialAnalysisExportModel(new Dictionary<ExportspectraType, IAnalysisExporter<ChromatogramPeakFeatureCollection>> {
                ...
                [ExportspectraType.centroid] = new AnalysisMgfExporter(file => new CentroidMsScanPropertyLoader(_storage.Parameter.ProviderFactoryParameter.Create(_providerFactory).Create(file), _storage.Parameter.MS2DataType)),
            })
            {
                FilePrefix = "Mgf",
                FileSuffix = "mgf",
