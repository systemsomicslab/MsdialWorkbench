    ...
    internal sealed class QuantMassEicLoader : IChromatogramLoader<Ms1BasedSpectrumFeature?>
    {
        ...
        async Task<PeakChromatogram> IChromatogramLoader<Ms1BasedSpectrumFeature?>.LoadChromatogramAsync(Ms1BasedSpectrumFeature? target, CancellationToken token) {
            if (target is not null) {
                ...
                var description = $"EIC of {peakFeature.Mass:N4} tolerance [Da]: {MzTolerance:F} Max intensity: {peakOfChromatogram?.Top.Intensity ?? 0d:F0}";
                return new PeakChromatogram(smoothed, peakOfChromatogram, string.Empty, Colors.Black, description);
            }
            return new PeakChromatogram(new Chromatogram([], _chromXType, _chromXUnit), null, string.Empty, Colors.Black);
        }
        ...
        PeakChromatogram IChromatogramLoader<Ms1BasedSpectrumFeature?>.EmptyChromatogram  => new PeakChromatogram(new Chromatogram([], _chromXType, _chromXUnit), null, string.Empty, Colors.Black);
