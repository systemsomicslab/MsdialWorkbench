...
internal sealed class AlignmentPeaksSpectraLoader
{
    ...
    public Task<IMSScanProperty?[]> GetCurrentScansAsync(IEnumerable<AnalysisFileBeanModel> files, AlignmentSpotPropertyModel target) {
        if (target is null) {
            return Task.FromResult(files.Select(_ => default(IMSScanProperty?)).ToArray());
        }
        return Task.WhenAll(files.Select(f => _loaders[f].LoadScanAsObservable(target).FirstAsync().ToTask()).ToArray());
    }
}
