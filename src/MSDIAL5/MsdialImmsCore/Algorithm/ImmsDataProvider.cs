...
namespace CompMs.MsdialImmsCore.Algorithm
{
    ...
    public class ImmsAccumulateDataProvider : BaseDataProvider
    {
        public ImmsAccumulateDataProvider(IEnumerable<RawSpectrum> spectrums, double massTolerance, double driftTolerance, double timeBegin, double timeEnd)
            : base(AccumulateSpectrum(FilterByScanTime(spectrums, timeBegin, timeEnd).ToList(), massTolerance, driftTolerance)) {

        }
        
        public ImmsAccumulateDataProvider(IEnumerable<RawSpectrum> spectrums, double massTolerance, double driftTolerance)
            : base(AccumulateSpectrum(spectrums.ToList(), massTolerance, driftTolerance)) {

        }

        public ImmsAccumulateDataProvider(RawMeasurement rawObj, double massTolerance, double driftTolerance, double timeBegin, double timeEnd)
            : this(rawObj.SpectrumList, massTolerance, driftTolerance, timeBegin, timeEnd) {

        }

        public ImmsAccumulateDataProvider(RawMeasurement rawObj, double massTolerance, double driftTolerance)
            : this(rawObj.SpectrumList, massTolerance, driftTolerance) {

        }

        public ImmsAccumulateDataProvider(RawMeasurement rawObj)
            :this(rawObj, 0.001, 0.002) { }

        public ImmsAccumulateDataProvider(AnalysisFileBean file, double massTolerance, double driftTolerance, double timeBegin, double timeEnd, bool isGuiProcess = false, int retry = 5)
            :this(LoadMeasurement(file, false, false, isGuiProcess, retry), massTolerance, driftTolerance, timeBegin, timeEnd) {

        }

        public ImmsAccumulateDataProvider(AnalysisFileBean file, bool isGuiProcess = false, int retry = 5)
            :this(LoadMeasurement(file, false, false, isGuiProcess, retry)) {

        }

        public ImmsAccumulateDataProvider(AnalysisFileBean file, double massTolerance, double driftTolerance, bool isGuiProcess = false, int retry = 5)
            :this(LoadMeasurement(file, false, false, isGuiProcess, retry), massTolerance, driftTolerance) {
    ...
    public sealed class ImmsAccumulateDataProviderFactory : IDataProviderFactory<AnalysisFileBean>, IDataProviderFactory<RawMeasurement>
    {
        private readonly bool _isGuiProcess;
        private readonly int _retry;
        private readonly double _massTolerance, _driftTolerance;
        ...
        public ImmsAccumulateDataProviderFactory(double massTolerance, double driftTolerance, double timeBegin, double timeEnd, int retry = 5, bool isGuiProcess = false) {
            _retry = retry;
            _isGuiProcess = isGuiProcess;
            _massTolerance = massTolerance;
            _driftTolerance = driftTolerance;
        ...
        public ImmsAccumulateDataProviderFactory(double massTolerance, double driftTolerance, int retry = 5, bool isGuiProcess = false)
            :this(massTolerance, driftTolerance, double.MinValue, double.MaxValue, retry, isGuiProcess) {
        ...
        public IDataProvider Create(AnalysisFileBean file) {
            var measurement = file.LoadRawMeasurement(isImagingMsData: false, isGuiProcess: _isGuiProcess, retry: _retry, sleepMilliSeconds: 1000);
            return new ImmsAccumulateDataProvider(measurement, _massTolerance, _driftTolerance, _timeBegin, _timeEnd);
        }

        public IDataProvider Create(RawMeasurement rawMeasurement) {
            return new ImmsAccumulateDataProvider(rawMeasurement, _massTolerance, _driftTolerance, _timeBegin, _timeEnd);
        }
    }
}
