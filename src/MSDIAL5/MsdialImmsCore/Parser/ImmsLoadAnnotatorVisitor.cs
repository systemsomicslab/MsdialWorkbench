...
{
    public sealed class ImmsLoadAnnotatorVisitor : ILoadAnnotatorVisitor
    {
        ...
        public ISerializableAnnotator<IAnnotationQuery<MsScanMatchResult>, MoleculeMsReference, MsScanMatchResult, MoleculeDataBase> Visit((StandardRestorationKey key, MoleculeDataBase database) item) {
            if (item.key.SourceType.HasFlag(SourceType.MspDB)) {
                return new ImmsMspAnnotator(item.database, item.key.Parameter, Parameter.TargetOmics, item.key.Key, item.key.Priority);
            }
            else if (item.key.SourceType.HasFlag(SourceType.TextDB)) {
                return new ImmsTextDBAnnotator(item.database, item.key.Parameter, item.key.Key, item.key.Priority);
            }
        ...
        public ISerializableAnnotator<IAnnotationQuery<MsScanMatchResult>, MoleculeMsReference, MsScanMatchResult, MoleculeDataBase> Visit((MspDbRestorationKey key, MoleculeDataBase database) item) {
            return new ImmsMspAnnotator(item.database, Parameter.MspSearchParam, Parameter.TargetOmics, item.key.Key, item.key.Priority);
        }
        ...
        public ISerializableAnnotator<IAnnotationQuery<MsScanMatchResult>, MoleculeMsReference, MsScanMatchResult, MoleculeDataBase> Visit((TextDbRestorationKey key, MoleculeDataBase database) item) {
            return new ImmsTextDBAnnotator(item.database, Parameter.TextDbSearchParam, item.key.Key, item.key.Priority);
        }
    ...
    public sealed class ImmsAnnotationQueryFactoryGenerationVisitor : IAnnotationQueryFactoryGenerationVisitor {
        ...
        public ImmsAnnotationQueryFactoryGenerationVisitor(PeakPickBaseParameter peakPickParameter, RefSpecMatchBaseParameter searchParameter, ProteomicsParameter proteomicsParameter, IMatchResultRefer<MoleculeMsReference, MsScanMatchResult> refer) {
