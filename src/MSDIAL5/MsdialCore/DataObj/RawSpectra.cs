...
public class RawSpectra : IRawSpectra
{
    ...
    private IChromatogramTypedSpectra BuildIfNotExists(ChromXType type, ChromXUnit unit) {
        return _spectraImpls.GetOrAdd((type, unit), pair => new Lazy<IChromatogramTypedSpectra>(() => BuildTypedSpectra(_spectra, pair.Item1, pair.Item2, _ionMode, AcquisitionType))).Value;
    }
