...
namespace CompMs.MsdialLcImMsApi.Process
{
    public sealed class FileProcess : IFileProcessor
    {
        ...
        public async Task RunAsync(AnalysisFileBean file, ProcessOption option, IProgress<int>? reporter = null, CancellationToken token = default) {
            ...
            var reportAction = reporter is not null ? reporter.Report : (Action<int>)null;
            ...
            var (chromPeakFeatures, mSDecResultCollections) = option.HasFlag(ProcessOption.PeakSpotting)
                ? FindPeakAndScans(file, spectrumProvider.Value, accSpectrumProvider, reportAction, token)
                : await LoadPeakAndScans(file, token).ConfigureAwait(false);

            if (option.HasFlag(ProcessOption.Identification)) {
                // annotations
                Console.WriteLine("Annotation started");
                await PeakAnnotationAsync(annotationProcess, spectrumProvider.Value, chromPeakFeatures.Items, mSDecResultCollections, storage.Parameter, reportAction, token).ConfigureAwait(false);
            }
        ...
        private (ChromatogramPeakFeatureCollection, MSDecResultCollection[]) FindPeakAndScans(AnalysisFileBean file, IDataProvider spectrumProvider, IDataProvider accSpectrumProvider, Action<int> reportAction, CancellationToken token) {
            Console.WriteLine("Peak picking started");
            var chromPeakFeatures_ = PeakSpotting(file, spectrumProvider, accSpectrumProvider, token, reportAction);
            ...
            var targetCE2MSDecResults = SpectrumDeconvolution(file, spectrumProvider, chromPeakFeatures_, summary, storage.Parameter, storage.IupacDatabase, reportAction, token);
