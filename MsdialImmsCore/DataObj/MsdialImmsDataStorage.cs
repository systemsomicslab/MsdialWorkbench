...
namespace CompMs.MsdialImmsCore.DataObj
{
    ...
    public class MsdialImmsDataStorage : MsdialDataStorageBase, IMsdialDataStorage<MsdialImmsParameter> {
        ...
        class MsdialImmsSerializer : MsdialSerializerInner, IMsdialSerializer
        {
            ...
            protected override async Task LoadDataBasesAsync(IStreamManager streamManager, string path, IMsdialDataStorage<ParameterBase> storage) {
                using (var stream = await streamManager.Get(path).ConfigureAwait(false)) {
                    storage.DataBases = DataBaseStorage.Load(stream, new ImmsLoadAnnotatorVisitor(storage.Parameter));
                }
            }
        }
    }
